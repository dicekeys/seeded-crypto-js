...
export interface RecipeForExpensiveHashFunctions {
    ...
    /**
     * The number of passes that Argon2id or Scrypt will need to make through that
...
export interface SeededCryptoSerializableObjectStatics<JsObjectFields, T> {
    /**
    ...
    * and is not automatically garbage collected by the JavaScript runtime.
     ...
     * generated by a toSerializedBinaryForm() on a prior instance of this class.
     * 
     * @returns The reconstituted object, which **you must delete manually** by
     * calling its [[delete]] method to avoid memory leaks.  That's because
     * derived objects are stored within the WebAssembly module
    ...
    /**
     * Reconstitute the object from its fields (a format which can be
     * transferred across origins via postMessage).
     * @param jsObj 
     */
    fromJsObject(jsObj: JsObjectFields): T;
}
interface DerivableFromSeed<JsObjectFields, T> extends SeededCryptoSerializableObjectStatics<JsObjectFields, T> {
    /**
...
interface SeededCryptoSerializableObject<JsObjectFields> extends ExplicitDelete {
    ...
    /**
     * Serialize this object into a JavaScript object that can be
     * transported across origins using postMessage.
     * The fields of the object are preserved, but the prototype
     * and all methods are removed so that the data within the
     * object can be consumed by code that does not use the
     * SeededCrypto library.  Secret objects can be consumed
     * directly, PackagedSealedMessages can be stored,
     * and other keys can be used by taking the value of the
     * raw key fields and using libSodium directly.
     * 
     * @returns This object serialized into a byte array.
     */
...
interface Unsealing {
   ...
   */
     ...
     * converted to JSON format: restoring it from JSON format, decrypting it, verifying that the
     * message hasn't been modified*, and that, if any unsealingInstructions were been
     * provided when the message was sealed, they also haven't changed. 
     * 
     * *The integrity check can prevent modifications of the ciphertext from being
    ...
    /**
     * Unseal a message that was previously sealed into a [[PackagedSealedMessage]] and
     * converted to binary format: restoring it from a byte array, decrypting it, verifying that the
     * message hasn't been modified*, and that, if any unsealingInstructions were been
     * provided when the message was sealed, they also haven't changed. 
     * 
     * *The integrity check can prevent modifications of the ciphertext from being
...
/**
 * The JSON encoding format of the Password class.
 * 
 * @category JSON Serialization Format
 */
 ...
 * @category SealingKey
 */
export interface SealingKey extends DerivedSecret<SealingKeyFields>, Sealing {
    /**
 ...
 * @category Secret
 */
export interface Secret extends DerivedSecret<SecretFields>, SecretFields
{}
 ...
 * @category SignatureVerificationKey
 */
export interface SignatureVerificationKey extends
    SignatureVerificationKeyFields,
    DerivedSecret<SignatureVerificationKeyFields>
{
    /**
 ...
 * @category SigningKey
 */  
 export interface SigningKey extends
    SigningKeyFields,
    Omit<DerivedSecret<SigningKeyFields>, "toCustomJson">
{
    /**
 ...
 * @category SymmetricKey
 */   
export interface SymmetricKey extends
    SymmetricKeyFields,
    DerivedSecret<SymmetricKeyFields>,
    Sealing, Unsealing
{}
 ...
 */
export interface UnsealingKeyStatic extends
 ...
 * @category UnsealingKey
 */
export interface UnsealingKey extends
    UnsealingKeyFields,
    DerivedSecret<UnsealingKeyFields>,
    Unsealing
{
