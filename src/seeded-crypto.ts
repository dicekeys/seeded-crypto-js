 ...
 * 
 * The DiceKeys Seeded Crypto Library for JavaScript and
 * (since only a masochist would use an untyped language) TypeScript
 * 
 * Because it uses WebAseembly, this module is packaged as a promise you need
 * to await to ensure the assembly code has been fully compiled by the JavaScript
 * engine before you use it.
 * 
 * To get access to the module, use
 * ```typescript
 * import {SeededCryptoModulePromise} from "seeded-crypto-js";

 * async function doSomeStuff() {
 *   const seededCryptoModule = await SeededCryptoModulePromise;
 * 
 *   const symmetrickey = seededCryptoModule.SymmetricKey.deriveFromSeed(seedString, derivationOptionJson);
 * }
 * ```
 * 
 * or
 ...
 * });
 * ```
 * A promise to a a web assembly module with our TypeScript helpers for allocating
...
interface SealedCryptoSerializableObjectStatics<T> {
    /**
 ...
 */
export interface StaticPackagedSealedmessage{
    /**
 ...
 * @category SealingKey
 */
export interface StaticSealingKey extends DerivedSecretStatics<SealingKey> {}

/**
 * A sealingKeyBytes is used to _seal_ messages, in combination with a
 ...
 * UnsealingKey and then call `getSealingKey()` on the UnsealingKey.
 *
 ...
 * @category Secret
 */
export interface StaticSecret extends DerivedSecretStatics<Secret> {}

/**
 * A secret derived from a seed string 
 ...
 */
export interface StaticSignatureVerificationKey extends DerivedSecretStatics<SignatureVerificationKey> {}
 ...
 * @category SigningKey
 */
export interface StaticSigningKey extends DerivedSecretStatics<SigningKey> {
 ...
 */
export interface StaticSymmetricKey extends DerivedSecretStatics<SymmetricKey>, StaticSealingAndUnsealing {}
 ...
 */
export interface StaticUnsealingKey extends DerivedSecretStatics<UnsealingKey>, StaticSealingAndUnsealing {
}
